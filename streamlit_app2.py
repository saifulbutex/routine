...
def load_my_model():
    ...
    if len(input_shape) != 4 or input_shape[3] != CHANNELS:
        raise ValueError(
            f"Expected model input shape to have 3 channels, got {input_shape}"
        )

    global TARGET_SIZE, MODEL_INPUT_SHAPE
    MODEL_INPUT_SHAPE = input_shape
    if input_shape[1] is None or input_shape[2] is None:
        raise ValueError(
            f"Model input shape must have fixed width and height, got {input_shape}"
        )
