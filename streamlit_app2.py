    ...
    try:
        with open(secrets_path, "r", encoding="utf-8") as f:
            if toml_parser is not None:
                ...
                st.write(f"Local secrets raw: {data}")
                return data.get(key)
            else:
                raise RuntimeError("No TOML parser available")
    except Exception as e:
        st.write(f"Local secret parse error ({type(e).__name__}): {e}")

    # Fallback: parse the file manually for the key
    try:
        with open(secrets_path, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip().startswith(f"{key} ") or line.strip().startswith(f"{key}="):
                    ...
                    if len(parts) == 2:
                        ...
                        st.write(f"Local secret fallback found: {raw_value}")
                        return raw_value
    except Exception as e:
        st.write(f"Local secret fallback parse error ({type(e).__name__}): {e}")
