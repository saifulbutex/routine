...
def load_my_model():
    ...
    input_shape = model.input_shape  # expected (None, 224, 224, 3)
    if len(input_shape) == 4 and input_shape[1:] != (TARGET_SIZE[0], TARGET_SIZE[1], CHANNELS):
        raise ValueError(
            f"Expected model input shape (None, {TARGET_SIZE[0]}, {TARGET_SIZE[1]}, {CHANNELS}), got {input_shape}"
        )
    return model
...
def preprocess_image(img_array):
    image_rgb = ensure_rgb(img_array)
    image_resized = cv2.resize(image_rgb, TARGET_SIZE, interpolation=cv2.INTER_AREA)
    image_norm = image_resized.astype("float32") * RESCALE_FACTOR
...
def process_image(img_array):
    ...
    try:
        ...
        preds = model.predict(image_batch, batch_size=BATCH_SIZE)
