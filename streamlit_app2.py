#model change + defect classes change = new setup
...
# --- 3. PIL-BASED RGB PREPROCESSING ---
# --- 3. ENVIRONMENT-ALIGNED PREPROCESSING ---
# --- 3. COLOR-STRIPPING PREPROCESSING ---
def preprocess_image(pil_image):
    """
    Converts colored fabrics into high-intensity grayscale arrays 
    to mimic the white fabric background the model was trained on.
    """
    open_cv_image = np.array(pil_image)
    open_cv_image = cv2.cvtColor(open_cv_image, cv2.COLOR_RGB2BGR)
    
    # 1. Strip all color by converting to Grayscale
    gray = cv2.cvtColor(open_cv_image, cv2.COLOR_BGR2GRAY)
    
    # 2. Resize to your model's required input size
    resized = cv2.resize(gray, IMG_SIZE)
    
    # 3. LIVE COLOR PATCH: Invert or intensely boost dark fabrics
    # If the image background is dark (colored), we normalize its brightness
    # to push the baseline pixels closer to a white/light-gray threshold (high values)
    resized = cv2.normalize(resized, None, alpha=150, beta=255, norm_type=cv2.NORM_MINMAX)
    
    # 4. Rebuild the 3-channel structure MobileNetV2 expects
