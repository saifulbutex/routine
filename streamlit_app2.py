...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="AI in Sewing: MobileNet Engine", layout="wide")

# File ID extracted from your shared link
DRIVE_FILE_ID = '1Mv0ZkFeSUlrw7iCxFcFGSORjy3SRxrMj'
MODEL_PATH = 'sewing_defect_mobilenet.keras'
IMG_SIZE = (224, 224)
...
def load_apparel_model():
    ...
    try:
        ...
        st.sidebar.success("✅ MobileNet Engine Ready")
        return model
    except Exception as e:
...
st.markdown("Automated defect detection using MobileNet and Grayscale Texture Analysis.")
st.divider()

st.sidebar.header("Scanner Settings")
input_mode = st.sidebar.radio("Input Source:", ("Upload Fabric Image", "Live Camera"))

img_source = None
if input_mode == "Upload Fabric Image":
    img_source = st.file_uploader("Select fabric sample", type=['jpg', 'jpeg', 'png'])
else:
    img_source = st.camera_input("Scan fabric sample")
